...
```{r WDIPrep, code = readLines('../Preprocessing/dataPrepper.R'), echo=FALSE, warning=FALSE, message=FALSE}
```
```{r happyPrep, code = readLines('../Preprocessing/happyPrepper.R'), echo=FALSE, warning=FALSE, message=FALSE}
```
...
For instance, we often hear that money can buy happiness, but is this true? It might also be that higher employment rates relate to a higher happiness score. Our curiosity to test these myths led us to analyzing this dataset. 
...
The merging of this data required subsetting the World Bank indicators to a small fraction of its size, according to constraints we imposed.
...
*But*, in actuality, the happiness score lives in another sheet. In that sheet, we have values for the *explainability* of all the indicators. That is, how much does each variable explain the happiness for that country? Our guess is that they have used the imputed values to calculate these explainabliity factors. 

Due to missing factors, later on when we report on how happiness correlates with the indicators from the World Bank, we will only be using the actual happiness values. However, we still measure the immediate correlation for all the happiness indicators. In this case, the missing values are simply ignored in the correlation calculation.

***

## 5 - How does happiness scores and indicators relate to the more specific indicators that the World Bank measures?
...
## 6 - How do certain World Bank indicators correlate with happiness values throughout the past 5 years?
...
### 6.1 - Positively Correlated Indicators
One clear indicator that has positively affected is **GDP**. We'll see how this is correlated with happiness throughout the past 5 years. Note that this just for the entire world, not individual countries.
...
p
```
How about that? Every single indicator shown above has shown a sharp downturn in how correlated it is with happiness. **Mortality rate**'s correlation has decreased by more than 16% since 2014! **Vulnerable employment** and the **age dependency ratio of young to employees** has also decreased quite a bit. However, we can note that some of them have experience an uptick on a year-to-year to basis, such as with vulnerable employment and the young to employee dependency ratio.
...
## 7 - How do happiness correlations differ from country to country?
...
To help visualize this, we’ve created an [interactive world map](https://bl.ocks.org/prajbabu/raw/64632ce0480ca5bdfdc03ebe55189d20/71d16abad6fb300447918e825cab23db20c43f21/) which allows you to select any country to see which World Bank development indicator was the most positively correlated and most negatively correlated to the happiness score for that country, for the 5-year time period between 2014 and 2018. You can see how these two indicators, along with happiness, change over this time. We've compiled our thoughts on three countries below, the United States, China, and Finland.
...
### 7.1 - United States
Upon clicking the United States, the line chart that pops up depicts the percent changes of the **personal remittances received** a.k.a. income received from nonresidents as a positively correlated indicator, the **labor force participation rate** as the negatively correlated indicator, and the USA's happiness. You will notice that even though the labor participation rate has increased over time, the happiness has decreased. Perhaps this is due to the large drop in personal remittances. This means that fewer people are dependent on money incoming from sources outside the United States, such as family living abroad, or employers abroad. As a result, it could be that current residents feel a bit more satisfied that they do not have to constantly rely on foreign income, and are now self-sufficient.
...
If you would like to find a description on any of the indicators, please refer to the [World Bank Open Data](https://data.worldbank.org/) page to look up the indicators.

***

## 8 - Conclusion and Future Work
During the course of this analysis, we faced a few limitations. Initially we anticipated to analyze values of the development indicates for a longer time frame i.e. 2000-2019, but we soon came to realize that the World Bank indicators dataset contains many missing values, as they did not have the means to measure many indicators until the more recent years. Also, the World Happiness Report did not publish annually until 2015, thus restraining our analysis to be between the 2015 and 2019 reports. In addition, for the data between the years 2014 - 2018, many countries were excluded from the analysis due to missing values, as these were developing countries that might not have policies in place yet to measure development indicators. 
...
From this project, we learned how to deal with large datasets and extract fields of interest for our analysis. We also learned how to merge datasets based on a key column (Country Name) and the problems that arise when different sources have different naming conventions. Finally, we gained experience with using a [TopoJSON](https://github.com/topojson/world-atlas) file that contains country coordinates, boundaries, and identifier values to plot an interactive world map using D3. 
